...
namespace Monocle
{
    public class Tween : Component
    {
        ...
        {
            Tween tween = null;
            foreach (var cachedTween in cached)
            {
                if (Engine.FrameCounter > cachedTween.cachedFrame + 3)
                {
                    tween = cachedTween;
                    cached.Remove(cachedTween);
                    break;
                }
            }
            if (tween == null)
        ...
        {
            Tween tween = Create(tweenMode, easer, duration, true);
        ...
        public override void Update()
        {
            ...
            Percent = Math.Max(0, TimeLeft) / Duration;
            if (!Reverse)
                Percent = 1 - Percent;
