...
namespace Monocle
{
    public static class Calc
    {
        ...
        public static string[] SplitLines(string text, SpriteFont font, int maxLineWidth, char newLine = '\n')
        {
            ...
            {
                string line = "";
                foreach (var word in forcedLine.Split(' '))
                {
        ...
        /// <summary>
        /// Returns a random Vector2, between min (inclusive) and max (exclusive).
        /// </summary>
        ...
        public static Color HexToColor(string hex)
        {
            ...
            {
                float r = (HexToByte(hex[offset]) * 16 + HexToByte(hex[offset + 1])) / 255.0f;
                float g = (HexToByte(hex[offset + 2]) * 16 + HexToByte(hex[offset + 3])) / 255.0f;
                float b = (HexToByte(hex[offset + 4]) * 16 + HexToByte(hex[offset + 5])) / 255.0f;
                return new Color(r, g, b);
            }
            else
            {
                int fromInt;
                if (int.TryParse(hex.Substring(offset), out fromInt))
                    return HexToColor(fromInt);
            }
        ...
        public static Color HsvToColor(float hue, float s, float v)
        {
            int hi = (int)(hue * 360f);
            float c = s * v;
            float x = c * (1f - Math.Abs((hi / 60f) % 2 - 1f));
            float m = v - c;
        ...
        public static T[][] VerifyLength<T>(this T[][] array, int length0, int length1)
        {
            ...
            for (int i = 0; i < array.Length; i++)
                array[i] = VerifyLength(array[i], length1);
            return array;
        }
        ...
        public static Vector2 FourWayNormal(this Vector2 vec)
        {
            ...
            vec = AngleToVector((float)Math.Floor((vec.Angle() + MathHelper.PiOver4) / MathHelper.PiOver2) * MathHelper.PiOver2, 1f);
        ...
        public static Vector2 EightWayNormal(this Vector2 vec)
        {
            ...
            vec = AngleToVector((float)Math.Floor((vec.Angle() + MathHelper.PiOver4 / 2f) / MathHelper.PiOver4) * MathHelper.PiOver4, 1f);
        ...
        {
            var angle = AngleApproach(vec.Angle(), targetAngleRadians, maxMoveRadians);
            return AngleToVector(angle, vec.Length());
        }
        ...
        public static int[,] ReadCSVIntGrid(string csv, int width, int height)
        {
            ...
            {
                string[] line = lines[y].Split(',', StringSplitOptions.RemoveEmptyEntries);
        ...
        /// <summary>
        /// Read positive-integer CSV with some tricks:
        ///  comma separated list of ints
        ///  "x-y" means from x to y inclusive
        ///  "x*n" means n repetitions of x
        /// </summary>
        ...
        {
            Debug.WriteLine(stopwatch.Elapsed + " : " + obj);
        }
        ...
        {
            var info = obj.GetType().GetMethod(method, BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
            if (info == null)
                return null;
            else
