    ...
    public class Sprite : Image
    {
        ...
        public override void Update()
        {
            if (Animating)
            {
                ...
                if (Math.Abs(animationTimer) >= currentAnimation.Delay)
                {
                    ...
                    {
                        string was = CurrentAnimationID;
                        if (OnLastFrame != null)
                            OnLastFrame(CurrentAnimationID);

                        // only continue if OnLastFrame didn't change the animation
                        if (was == CurrentAnimationID)
                        {
                            ...
                            else
                            {
                                ...
                                Animating = false;
                                string oldAnimationID = CurrentAnimationID;
                                CurrentAnimationID = "";
                                currentAnimation = null;
                                animationTimer = 0;
                                if (OnFinish != null)
                                    OnFinish(oldAnimationID);
                            }
                        }
                    }
                    else
        ...
        private MTexture[] GetFrames(string path, int[] frames = null)
        {
            ...
            else
            {
                ...
                ret = new MTexture[frames.Length];
        ...
        public void DrawSubrect(Vector2 offset, Rectangle rectangle)
        {
            if (Texture != null)
            {
                ...
                Vector2 sub = new Vector2(-Math.Min(rectangle.X - Texture.DrawOffset.X, 0), -Math.Min(rectangle.Y - Texture.DrawOffset.Y, 0));
                Draw.SpriteBatch.Draw(Texture.Texture.Texture, RenderPosition + offset, clip, Color, Rotation, Origin - sub, Scale, Effects, 0);
            }
        }
