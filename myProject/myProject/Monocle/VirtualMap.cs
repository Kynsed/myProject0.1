    ...
    public class VirtualMap<T>
    {
        ...
        public T this[int x, int y]
        {
            get
            {
                int cx = x / SegmentSize;
                int cy = y / SegmentSize;
                T[,] segment = segments[cx, cy];
                if (segment == null)
                    return EmptyValue;
                return segment[x - cx * SegmentSize, y - cy * SegmentSize];
            }
            set
            {
                int cx = x / SegmentSize;
                ...
                if (segments[cx, cy] == null)
                {
                    ...
                    // Preenche segmento novo com EmptyValue, se não for default(T)
                    if (EmptyValue != null && !EmptyValue.Equals(default(T)))
                    {
                        for (int i = 0; i < SegmentSize; i++)
                            for (int j = 0; j < SegmentSize; j++)
                                segments[cx, cy][i, j] = EmptyValue;
                    }
                }
